...
func _run()->void:
	...
	var forest_party:=campaign.default_party("forest");campaign.begin_expedition(forest_party,"forest","strategy");campaign.record_casualty(forest_party[0],"Acceptance-test casualty");campaign.expedition.carried_gold=80;campaign.expedition.carried_relic_essence=20;campaign.resolve_expedition("victory");_expect(campaign.memorial.size()==1 and campaign.retired_heroes.size()==1 and campaign.banked_gold==80 and campaign.candidate_pool.size()>=2,"Partial casualty, victory retirement, or candidate-wave resolution failed",failures)
