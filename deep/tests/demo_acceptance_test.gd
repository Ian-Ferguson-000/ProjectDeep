...
func _run()->void:
	...
	_expect(GameBalance.get_dungeons().size()==8 and GameBalance.get_dungeon_order().size()==8,"Demo must contain six regular and two secret dungeons",failures)
	...
	var campaign:=CampaignState.new();campaign.tutorial_phase=CampaignState.TUTORIAL_COMPLETE;campaign.ensure_roster();_expect(campaign.living_roster().size()==2,"Initial company roster is not two",failures)
	var forest_party:=campaign.default_party("forest");campaign.begin_expedition(forest_party,"forest","strategy");campaign.record_casualty(forest_party[0],"Acceptance-test casualty");campaign.expedition.carried_gold=80;campaign.expedition.carried_relic_essence=20;campaign.resolve_expedition("victory");_expect(campaign.memorial.size()==1 and campaign.living_roster().size()==1 and campaign.banked_gold==80 and campaign.candidate_pool.size()>=2,"Partial casualty, returning survivor, or candidate-wave resolution failed",failures)
	...
	campaign.record_dungeon_clear("forest","strategy");campaign.record_dungeon_clear("forest","slasher");campaign.record_dungeon_clear("ashen_farmstead","strategy");campaign.record_dungeon_clear("crypt","slasher");campaign.record_dungeon_clear("balors_hell","slasher");campaign.record_dungeon_clear("sunken_mine","strategy");campaign.record_dungeon_clear("ember_foundry","slasher");_expect(campaign.unlocked_classes.size()==6,"All six classes did not unlock through the campaign graph",failures)
	...
	var round_trip:=CampaignState.new();round_trip._load_dict(CampaignState._migrate_dict(campaign.to_dict()));_expect(round_trip.completed_dungeon_modes.size()==8 and round_trip.banked_relics.size()==4 and round_trip.memorial.size()==1,"Final campaign save round trip lost progression",failures)
