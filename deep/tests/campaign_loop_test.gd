...
func _run() -> void:
	...
	_expect(campaign.living_roster().size()==1 and campaign.memorial.size()==1 and campaign.retired_heroes.is_empty() and campaign.candidate_pool.size()>=2,"casualty, returning survivor, or replacement wave failed",failures)
