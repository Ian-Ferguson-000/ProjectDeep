...
func _run() -> void:
	...
	var interaction_chest:=SlasherBreakableProp.new();interaction_chest.setup("chest",Vector2i(3,4));root.add_child(interaction_chest);await process_frame;_expect(interaction_chest.is_in_group("slasher_chest"),"Closed chest is missing its interaction group",failures);_expect(interaction_chest.open_chest(),"Unlocked chest did not open",failures);_expect(interaction_chest.is_open and not interaction_chest.is_in_group("slasher_damageable") and not interaction_chest.open_chest(),"Chest opening was not exactly once",failures);interaction_chest.queue_free()
	for seed in range(100,150):
		...
		for spawn in layout.enemy_spawns:_expect(layout.cells.has(spawn),"Seed %d placed enemy outside floor"%seed,failures)
	...
	_expect(boss_layout.is_boss_floor and boss_layout.enemy_spawns.size()==1,"Floor eight must produce one campaign boss",failures)
	...
	for direction in [Vector2i.UP,Vector2i.RIGHT,Vector2i.DOWN,Vector2i.LEFT]:
		var boundary:=SlasherForestArt.make_boundary_sprite(direction,3);_expect(boundary.texture!=null,"Missing Verdant boundary art for %s"%direction,failures);boundary.free()
	...
	for visual_id in ["feral_wolf","poison_ranger","spore_beast","fire_mage","ice_mage","dark_druid"]:
	...
	var tuned_enemy:=SlasherEnemy.new();tuned_enemy.configure(3,false,"feral_wolf");_expect(tuned_enemy.elite and tuned_enemy.max_health==39 and tuned_enemy.damage==7 and tuned_enemy.reward==5,"Forest elite did not consume balance scaling",failures);tuned_enemy.free()
	var tuned_mini_boss:=SlasherEnemy.new();tuned_mini_boss.configure(5,false,"spore_beast",true);_expect(tuned_mini_boss.elite and tuned_mini_boss.max_health>80 and tuned_mini_boss.reward==12,"Forest elite miniboss did not consume dedicated scaling",failures);tuned_mini_boss.free()
	var tuned_boss:=SlasherEnemy.new();tuned_boss.configure(5,true,"dark_druid");_expect(tuned_boss.max_health==165 and tuned_boss.damage==16 and tuned_boss.reward==18,"Forest boss did not consume balance scaling",failures);tuned_boss.free()
	for class_id in GameBalance.get_base_classes().keys():
		...
		if class_id=="warrior":
			...
			var charger:=SlasherEnemy.new();charger.configure(3,false,"wolf_charger",false,"wolf_charger");charger.process_mode=Node.PROCESS_MODE_DISABLED;scene.actor_layer.add_child(charger);charger.target=scene.player;charger.global_position=scene.player.global_position+Vector2(200,0);var sampled_target:=scene.player.global_position;charger._process_charger(0.0);_expect(charger.ai_state=="windup" and charger.captured_target==sampled_target,"Charger did not snapshot its straight-line charge target",failures)
			...
			lurker.queue_free();charger.queue_free();howler.queue_free();wounded_wolf.queue_free();wolfmaster.queue_free()
		...
		for enemy_node in scene.get_tree().get_nodes_in_group("slasher_enemy"):
			if enemy_node is SlasherEnemy:
				...
				break
		if class_id=="summoner":scene.player._ensure_companion();_expect(is_instance_valid(scene.player.companion),"Summoner wolf was not created",failures)
