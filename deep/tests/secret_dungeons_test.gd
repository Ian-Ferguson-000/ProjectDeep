...
func _run()->void:
	...
	for pair in [["moonlit_grove",GROVE],["abyssal_archive",ARCHIVE]]:
		if campaign.default_party(String(pair[0])).is_empty():for candidate in campaign.get_candidates():campaign.recruit_candidate(candidate.id)
		var id:String=pair[0];_expect(campaign.can_unlock_secret(id),"%s clue/research unlock failed"%id,failures);var party:=campaign.default_party(id);campaign.begin_expedition(party,id,"strategy");var state:=RunState.new();state.attach_campaign(campaign);state.active_character_id=party[0];state.start_new_run(null,id,"strategy");var scene:Node=pair[1].instantiate();scene.setup(null,state);root.add_child(scene);await process_frame;_expect(scene.dungeon_id==id and scene.dungeon_merchant.is_empty() and not scene.enemies.is_empty(),"%s did not generate as a merchant-free secret runtime"%id,failures);scene.free();campaign.resolve_expedition("death")
